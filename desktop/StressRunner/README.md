...
## График загрузки (HWiNFO)
1. Положи HWiNFO в `StressTests\HWinfo\HWiNFO64.EXE` — приложение **само запустит
   его** при старте (или укажи путь в `Others\settings.json` → `hwinfo_path`).
2. В настройках HWiNFO включи **Settings → Shared Memory Support** (или добавь
   нужные сенсоры в Gadget кнопкой-значком). Тогда приложение читает их из
   реестра и рисует живой график CPU/GPU нагрузки и температур.
