      ...
      <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/partners")} title="К кабинету партнёра"
              className="flex h-9 w-9 items-center justify-center rounded-lg border border-border text-foreground/60 hover:border-primary hover:text-primary transition-colors" style={{ cursor: "pointer" }}>
            ...
            <button onClick={() => navigate("/")} className="flex items-center gap-2" style={{ cursor: "pointer" }}>
              ...
              <span className="font-semibold text-lg text-foreground">StressTester</span>
            </button>
          </div>
  ...
  if (!hasLk) {
    ...
    return shell(
      <Centered
        ...
        title={suspended ? "Доступ приостановлен" : company ? "Стресс-тесты недоступны" : "Компания не привязана"}
        text={
          ...
          : company ? "У вашей компании нет доступа к стресс-тестам. Свяжитесь с менеджером BeGraphics."
    ...
    <>
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-foreground/50">Стресс-тесты вашей компании</p>
        </div>
