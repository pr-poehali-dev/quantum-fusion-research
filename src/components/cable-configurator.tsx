...
// Отдельная палитра для 12VHPWR (12V-2x6)
const PALETTE_12V = [
  { id: "black",   label: "Чёрный",    hex: "#1a1a1a" },
  { id: "white",   label: "Белый",     hex: "#f0f0f0" },
  { id: "gray",    label: "Серый",     hex: "#6b7280" },
...
function CableConnector({ label, color, pattern }: { label: string; color: string; pattern?: boolean }) {
  return (
    <g>
      ...
      {pattern && (
        <rect width={36} height={14} rx={3} fill="url(#stripe)" opacity={0.3} />
      )}
...
// CPU кабели — сверху вниз, рядом
function CpuCables({ type, color, hexColor, onClick }: { type: CpuType; color: string; hexColor: string; onClick: () => void }) {
...
// 24-pin ATX — справа налево (горизонтальный)
function AtxCable({ color, hexColor, onClick }: { color: string; hexColor: string; onClick: () => void }) {
  return (
    <g onClick={onClick} style={{ cursor: "pointer" }} className="group">
      {/* Провода горизонтально */}
...
// GPU кабели — снизу вверх
function GpuCables({ type, color, hexColor, onClick }: { type: GpuType; color: string; hexColor: string; onClick: () => void }) {
  const is12v = type === "12V-2x6"
  const cables = type === "8-pin" ? ["8-pin"]
    : type === "8+8-pin" ? ["8-pin", "8-pin"]
    : type === "8+8+8-pin" ? ["8-pin", "8-pin", "8-pin"]
    : ["12V-2x6"]
...
export function CableConfigurator() {
  ...
  const [cables, setCables] = useState<CableState>({
    cpu: "8-pin", gpu: "8-pin",
    cpuColor: "black", gpuColor: "black", atxColor: "black",
  })
        ...
        <div className="border-t border-border px-5 pb-5 pt-4 space-y-5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {GPU_TYPES.map(t => (
                  <button key={t} onClick={() => {
                    setCables(p => ({ ...p, gpu: t, gpuColor: t === "12V-2x6" && !PALETTE_12V.find(c => c.id === p.gpuColor) ? "black" : p.gpuColor }))
                  }}
          ...
          <div className="rounded-xl border border-border/60 bg-background/40 p-3">
            ...
            <div className="flex justify-center overflow-x-auto">
              <svg width={svgWidth} height={220} viewBox={`0 0 ${svgWidth} 220`} style={{ maxWidth: "100%" }}>
                ...
                {/* ── CPU кабели — сверху, слева ── */}
                <g transform="translate(36, 90)">
                  <CpuCables type={cables.cpu} color={cables.cpuColor} hexColor={cpuHex}
                ...
                {/* ── ATX 24-pin — справа налево ── */}
                <g transform={`translate(${svgWidth - 20}, 80)`}>
                  <AtxCable color={cables.atxColor} hexColor={atxHex}
                ...
                <g transform={`translate(${svgWidth / 2 - (gpuCableCount * 44) / 2}, 122)`}>
                  <GpuCables type={cables.gpu} color={cables.gpuColor} hexColor={gpuHex}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {getPalette().map(color => {
                  const currentKey = activeColorKey ? cables[activeColorKey as keyof CableState] : null
                  const isActive = currentKey === color.id
                  return (
                    <button
                      key={color.id}
                      title={color.label}
          ...
          <div className="space-y-2">
            ...
            <button
              ...
              style={{ cursor: "pointer" }}
            >
              {added ? <span className="flex items-center justify-center gap-2"><Icon name="Check" size={15} />Добавлено в корзину</span>
