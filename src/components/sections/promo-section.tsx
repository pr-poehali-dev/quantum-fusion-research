...
export function PromoSection() {
  ...
  useEffect(() => {
    api.promos.getPublic()
      ...
      .catch(() => {})
    // Подстраховка: секция находится в самом низу scroll-snap контейнера,
    // где IntersectionObserver (root: null) может не сработать. Гарантированно
    // показываем контент, чтобы акции не оставались невидимыми.
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-5xl">
        <div className={`mb-10 transition-all duration-700 ${show ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"}`}>
        ...
        {promos.length === 0 ? (
          <div className={`rounded-2xl border border-dashed border-foreground/15 py-16 text-center transition-all duration-700 delay-100 ${show ? "translate-y-0 opacity-100" : "translate-y-6 opacity-0"}`}>
          ...
          <div className={`grid gap-4 sm:grid-cols-2 transition-all duration-700 delay-100 ${show ? "translate-y-0 opacity-100" : "translate-y-6 opacity-0"}`}>
            {promos.map(p => (
        ...
        <div className={`mt-6 flex flex-wrap items-center gap-x-3 gap-y-2 text-sm transition-all duration-700 delay-200 ${show ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"}`}>
