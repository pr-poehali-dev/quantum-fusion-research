...
export const api = {
  ...
  orders: {
    ...
    batchAddGroup: (id: number, data: { label?: string; qty?: number; components?: unknown[]; wants_assembly?: boolean }) => fetch(URLS.orders, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action: "batch_add_group", ...data }) }).then(r => r.json()),
    batchUpdateGroup: (id: number, group_id: number, data: { label?: string; qty?: number; components?: unknown[]; wants_assembly?: boolean }) => fetch(URLS.orders, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action: "batch_update_group", group_id, ...data }) }).then(r => r.json()),
