...
export const api = {
  ...
  orders: {
    ...
    batchAddGroup: (id: number, data: { label?: string; qty?: number; components?: unknown[]; wants_assembly?: boolean; assembly_type?: "percent" | "manual"; assembly_fee_manual?: number }) => fetch(URLS.orders, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action: "batch_add_group", ...data }) }).then(r => r.json()),
    batchUpdateGroup: (id: number, group_id: number, data: { label?: string; qty?: number; components?: unknown[]; wants_assembly?: boolean; assembly_type?: "percent" | "manual"; assembly_fee_manual?: number }) => fetch(URLS.orders, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, action: "batch_update_group", group_id, ...data }) }).then(r => r.json()),
  ...
  priceMonitor: {
    ...
    rejectAll: (adminKey: string, kind?: string) =>
  ...
  warehouse: {
    ...
    exportExcel: () => fetch(`${URLS.warehouse}?action=export_excel`).then(r => r.json()),
    ...
    createCategory: (name: string) => fetch(URLS.warehouse, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "category_create", name }) }).then(r => r.json()),
    reorderCategories: (names: string[]) => fetch(URLS.warehouse, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "category_reorder", names }) }).then(r => r.json()),
  ...
  stress: {
    ...
    // Что программа получает при проверке обновлений (публично, без ключа).
    launcherVersion: (channel: "full" | "lite" = "full") =>
      fetch(`${URLS.stress}?action=launcher_version&channel=${channel}`).then(r => r.json()),
    ...
    lastPublicRun: () =>
      fetch(`${URLS.stress}?action=last_public_run`).then(r => r.json()),
    ...
    brandArchive: (adminKey: string, auth?: StressAuth) =>
      fetch(`${URLS.stress}?action=brand_archive${stressQ(adminKey, auth)}`, { headers: stressHeaders(adminKey, auth) }).then(r => r.json()),
  ...
  stressReleases: {
    ...
    // version/edition — чтобы имя файла собрал бэкенд (переименовать
    // установщик в хранилище потом нельзя)
    getUploadUrl: (fileName: string, adminKey: string, version?: string, edition?: string) =>
      fetch(URLS.stressReleases, { method: "POST", headers: { "Content-Type": "application/json", "X-Admin-Key": adminKey }, body: JSON.stringify({ action: "upload_url", file_name: fileName, version, edition, ak: adminKey }) }).then(r => r.json()),
    ...
    resolveLink: (link: string, adminKey: string) =>
      fetch(URLS.stressReleases, { method: "POST", headers: { "Content-Type": "application/json", "X-Admin-Key": adminKey }, body: JSON.stringify({ action: "resolve_link", link, ak: adminKey }) }).then(r => r.json()),
    checkUpload: (s3Key: string, adminKey: string) =>
      fetch(URLS.stressReleases, { method: "POST", headers: { "Content-Type": "application/json", "X-Admin-Key": adminKey }, body: JSON.stringify({ action: "check_upload", s3_key: s3Key, ak: adminKey }) }).then(r => r.json()),
