...
PROMPT = (
    ...
    "ВАЖНО про qty (количество): бери его ТОЛЬКО из отдельной колонки "
    "«Количество» / «Объём» / «Кол-во» (обычно последняя числовая колонка строки). "
    "НИКОГДА не бери количество из НАЗВАНИЯ товара. В названии часто есть упаковка "
    "вида «20 шт/кор», «(20 шт/уп)», «по 50 шт», «1*120mm» — это НЕ количество к приёмке, "
    "это характеристика товара, оставь её внутри name и НЕ используй как qty. "
    "Пример: «Кулер ... (20 шт/кор, TDP 220W)  ...  137»  ->  qty=137 (а не 20).\n"
    "Другие правила: price — сумма по строке (цена×кол-во) числом без пробелов и валюты; "
    "не выдумывай товары, бери только реальные строки чека; "
    "служебные строки (ИТОГО, НДС, скидка, коды единиц измерения вроде «796 шт») "
    "НЕ включай в items."
)
