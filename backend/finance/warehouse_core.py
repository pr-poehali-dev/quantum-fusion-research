...
Копия из backend/reserves/warehouse_core.py — для использования внутри finance функции.
"""
...
def reserve_line(cur, order_id, product_id, qty, slot=None):
    ...
    # free = Σqty (qty уже свободное к резерву; qty_reserved НЕ вычитаем —
    # иначе двойное вычитание → ложный минус-резерв при наличии). Исправлено.
