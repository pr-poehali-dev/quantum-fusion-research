...
КВАНТ (ещё меньше VRAM): запусти этот же скрипт через start_worker_3b_q4.bat —
он задаёт RECEIPT_MODEL=qwen2.5vl:3b-q4_K_M (та же 3B, но ~2-3 ГБ, вся на GPU).
...
# По умолчанию — полная 3B. Для экономии VRAM используй квант: RECEIPT_MODEL=qwen2.5vl:3b-q4_K_M
MODEL = os.environ.get("RECEIPT_MODEL", "qwen2.5vl:3b")   # ← модель этого воркера (быстрая, 3B)
...
NUM_GPU = int(os.environ.get("RECEIPT_NUM_GPU", "999"))   # ВСЕ слои на видеокарту (Ollama положит максимум). Раньше 36 + большой контекст могли не влезть → часть на CPU (тормоз)
