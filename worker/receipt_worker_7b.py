...
КВАНТ для 12 ГБ VRAM: запусти этот же скрипт через start_worker_7b_q4.bat —
он задаёт RECEIPT_MODEL=qwen2.5vl:7b-q4_K_M (та же 7B, но ~5-6 ГБ, вся на GPU).
Модель/контекст/слои переопределяются переменными окружения:
RECEIPT_MODEL, RECEIPT_NUM_CTX, RECEIPT_NUM_GPU, RECEIPT_NUM_THREAD.
"""
...
# ─────────────────── НАСТРОЙКИ ───────────────────
# Модель/контекст можно переопределить через переменные окружения (см. .bat).
# По умолчанию — полная 7B. Для 12 ГБ VRAM используй квант: RECEIPT_MODEL=qwen2.5vl:7b-q4_K_M
MODEL = os.environ.get("RECEIPT_MODEL", "qwen2.5vl:7b")   # ← модель этого воркера
NUM_CTX = int(os.environ.get("RECEIPT_NUM_CTX", "8192"))  # длина контекста. 8k с запасом хватает на счёт; больше — лишний жор VRAM, из-за которого слои сваливаются на CPU
KEEP_ALIVE = -1                       # -1 = держать модель в VRAM вечно
NUM_GPU = int(os.environ.get("RECEIPT_NUM_GPU", "999"))   # ВСЕ слои на видеокарту (Ollama положит максимум). Раньше 28 + большой контекст не влезали → часть уходила на CPU (тормоз)
NUM_THREAD = int(os.environ.get("RECEIPT_NUM_THREAD", "2"))  # потоков CPU отдаём Ollama (у нас слабый 2-ядерный проц — больше не нужно)
...
def _page_has_items(text: str) -> bool:
    ...
    low = text.lower()
    # маркеры шапки товарной таблицы
    header_hits = sum(k in low for k in ("наименование", "кол-во", "количество", "цена", "сумма", "товар"))
    # маркеры мусорных страниц (оферта, образец платёжки, реквизиты)
...
def pdf_extract_text(data: bytes) -> str:
    ...
    if kept:
        ...
        # фильтр ничего не нашёл — берём всё, чтобы не потерять данные
